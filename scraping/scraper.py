...
def scrape_news(disease):
    ...
    for article in soup.find_all("div", class_="SoaBEf"):
        ...
        news_data.append((news_timestamp.strftime("%Y-%m-%d %H:%M"), location, disease, severity))
...
def run_scraper():
    print("\n🕒 Starting Kerala Disease News Scraper (Runs every 2 hours)...")
    ...
    if all_news:
        ...
        print(f"✅ {len(all_news)} records saved to {OUTPUT_FILE}.\n")
    else:
